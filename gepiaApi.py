...
class correlation:
    ...
    def query(self):
        ...
        if json.loads(ret)['status']==0:
            ...
            self.url=GEPIA+'tmp/'+pdf
            request.urlretrieve (self.url, self.out_dir+pdf)
            fname = 'corr' +'_'+''.join(self.params['signature1'])+'_'+''.join(self.params['signature2'])+'_'+ self.params['dataset']+'_'+self.params['signature1_norm']+'_'+pdf.split('_')[3]
            os.rename(self.out_dir+pdf,self.out_dir+fname)
            return self.out_dir+pdf
        else:
            print('Error! Please check your parameters.')
            self.showParams()
...
class boxplot:
    ...
    def query(self):
        ...
        if json.loads(ret)['outdir']!='fail':
            ...
            self.url=GEPIA+'tmp/'+pdf
            request.urlretrieve (self.url, self.out_dir+pdf)
            fname = pdf.split('_')[0]+'_'+self.params['dataset'][0]+'_'+self.params['matchdata']+'_'+pdf.split('_')[2]
            os.rename(self.out_dir+pdf,self.out_dir+fname)
            return self.out_dir+pdf
        else:
            print('Error! Please check your parameters.')
...
class survival:
    ...
    def query(self):
        ...
        if json.loads(ret)['outdir']!='fail':
            ...
            self.url=GEPIA+'tmp/'+pdf
            request.urlretrieve (self.url, self.out_dir+pdf)
            fname = '_'.join(self.params['signature'])+'_'+self.params['dataset'][0]+'_'+self.params['methodoption']+'_'+self.params['groupcutoff1']+'_'+self.params['signature_norm']+'_'+pdf.split('_')[2]
            os.rename(self.out_dir+pdf,self.out_dir+fname)
            return self.out_dir+pdf
        else:
            print('Error! Please check your parameters.')
...
class survival_map:
    ...
    def query(self):
        ...
        if json.loads(ret)['outdir']!='fail':
            ...
            self.url=GEPIA+'tmp/'+pdf
            request.urlretrieve (self.url, self.out_dir+pdf)
            fname = 'survmap'+'_'+self.params['signature'][0]+'_'+self.params['methodoption']+'_'+self.params['groupcutoff1']+'_'+pdf.split('_')[2]
            os.rename(self.out_dir+pdf,self.out_dir+fname)
            return self.out_dir+pdf
        else:
            print('Error! Please check your parameters.')
